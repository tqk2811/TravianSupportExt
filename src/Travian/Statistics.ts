function StatisticsReader(): void {
...
function StatisticsRender(): void {
    if (window.Instance.isPlus && window.location.pathname.startsWith("/village/statistics")) {
        let maintab = window.Instance.e_ActiveTabMain;
        if (maintab) {
            let href = maintab.getAttribute("href");
            switch (href) {
                case "/village/statistics/overview":
                    {
                        NumCountAtt("att1", "color:red;");
                        NumCountAtt("att3", "color:#E6E6FA");
                        $("unit").each(function (index, element) {
                            let e = $(element);
                            let alt = e.attr("alt");
                            if (alt) {
                                let count = alt.match(/\dx/);
...
function NumCountAtt(className: string, color: string){
